...
class Card:
    ...
    def render_card_row(self, card_row_str):
        ...
        for ch in card_row_str:
            if ch in ('╭', '─', '╮', '│', '╰', '╯'):
                rendered_str += (MAGENTA if self.is_highlighted else (YELLOW if self.is_selected else GREEN) ) + ch + RESET
...
class Pile:
    ...
    def show_cards(self, direction='horizontal', compact=False):
        if direction == 'horizontal':
            all_cards = [''] * 8
            each_card_starts_at_col = [0]
            for row_index in range(1, 7):
                concated_segments = []
                card_count = len(self.cards)
                for card_index, card in enumerate(self.cards):
                    if card.face_up:
                        full_row = card.get_dot_matrix()[row_index-1 if not card.is_selected else row_index]
                    else:
                        full_row = Card.face_down_card_temp[row_index-1 if not card.is_selected else row_index]
                    
                    if not compact:
                        concated_segments.append(full_row)
                        if row_index == 1:
                            each_card_starts_at_col.append(each_card_starts_at_col[-1] + len(full_row))
                    else:
                        if card_index != card_count-1:
                            ...
                            col_span = 9
                        cropped_row = full_row[:col_span]
                        concated_segments.append(cropped_row)
                        if row_index == 1:
...
class Game:
    ...
    def human_set_cards_to_play(self, cards, valid_moves, must_play):
        if not cards:
            return None
        length = len(cards)
        cards[0].set_highlighted(True) # initially highlight the first card
        highlight_index = 0
        self.display()
        ...
        while True:
            ...
            if key == 'left':
                ...
                cards[highlight_index].set_highlighted(True)
            elif key == 'right':
                cards.clear_highlight()
                highlight_index = (highlight_index + length + 1) % length
                cards[highlight_index].set_highlighted(True)
            elif key == 'select':
                cards[highlight_index].set_selected(True)
            elif key == 'unselect':
                cards[highlight_index].set_selected(False)
            elif key == 'toggle_select':
                cards[highlight_index].toggle_selected()
            elif key == 'clear':
                cards.clear_selection()
            elif key == 'play':
                ...
                if selected_cards == []:
                    ...
                    # 检查选中的牌是否合法
                    check_valid = False
                    selected_repr = set([str(card) for card in selected_cards])
                    for moves in valid_moves:
                        card_repr = set([str(card) for card in moves])
                        if selected_repr == card_repr:
                            check_valid = True
                            break
                    if not check_valid:
                        print("选中的牌不能出, 请重新选择, 直接[回车]/[0]不出牌, [T]获取帮助")
                        continue
                    else:
                        # 离开前重置样式
                        cards.clear_highlight()
                ...
                cards.clear_highlight()
                cards.clear_selection()
                self.display() # 离开前刷新一次界面
                return 'help'
            
            self.display()

    def human_select_moves_to_play(self, valid_moves, must_play):
        print("可出的牌:")
        if not must_play:
            print("0. ==不出==")
        ...
        while True:
            ...
            if current_player.is_human:
                ...
                if not valid_moves:
                    ...
                    ## 两种交互模式
                    chosen_move = self.human_set_cards_to_play(current_player.cards, valid_moves, must_play)
                    if chosen_move == 'help':
                        chosen_move = self.human_select_moves_to_play(valid_moves, must_play)
                    
                    if not chosen_move: # 返回[]的时候
                        self.pass_count += 1
                        self.player_last_moves[self.current_player_index].refresh_cards([]) # 相当于确认了这轮操作, 不出牌
                    else:
